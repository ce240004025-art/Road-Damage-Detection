...
function Dashboard() {
  ...
  const totalScore = myReports.reduce((total, report) => total + (report.points || 0), 0);
      ...
      <div className="page-container animate-fade-in">
        ...
        <div className="glass-card" style={{ padding: '20px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
          <h1 style={{ margin: 0, fontSize: '28px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            Road Damage <span style={{ color: 'var(--accent-primary)', marginLeft: '6px' }}>Scanner</span>
          </h1>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <span style={{ fontWeight: '500', color: 'var(--text-secondary)' }}>
              Hi, <span style={{ color: 'white' }}>{username}</span>
            </span>
            <button className="btn btn-primary" onClick={() => navigate("/upload")}>
              <Plus size={18} /> New Scan
            </button>
            <button className="btn btn-secondary" onClick={handleLogout} style={{ padding: '14px' }}>
              <LogOut size={18} />
            </button>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 3fr', gap: '32px' }}>
          ...
          <div className="animate-fade-up delay-100">
            <div className="glass-card glass-card-compact hover-lift" style={{ marginBottom: '24px' }}>
              <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', color: 'var(--text-secondary)', fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px' }}>
            ...
            <div className="glass-card" style={{ padding: '24px' }}>
              ...
              {loading ? (
                <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-secondary)' }}>
                  <RefreshCw className="pulse-circle" size={32} style={{ margin: '0 auto', background: 'transparent' }} />
                  <p>Syncing database...</p>
                </div>
              ) : reports.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '80px 20px', background: 'rgba(255,255,255,0.01)', borderRadius: '16px', border: '1px dashed var(--glass-border)' }}>
                  <ImageIcon size={48} color="var(--text-secondary)" style={{ marginBottom: '16px', opacity: 0.5 }} />
                  <h3 style={{ marginBottom: '8px' }}>No uploads found</h3>
                  <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>
                    {view === "my" ? "You haven't uploaded any road damage photos yet." : "No public uploads available."}
                  </p>
                  {view === "my" && (
                    <button className="btn btn-primary" onClick={() => navigate("/upload")}>
                      Start Uploading
                    ...
                    <div key={r.id} className={`glass-card glass-card-compact hover-lift animate-fade-up delay-${(i%3 + 1)*100}`}>
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>by @{r.username}</span>
                          <span className={`badge ${getConfidenceClass(r.confidence)}`}>
                            {r.confidence ? `${(r.confidence * 100).toFixed(0)}% Match` : "N/A"}
                          </span>
                        </div>
                      </div>
